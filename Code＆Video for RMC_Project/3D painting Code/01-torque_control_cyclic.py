...
class TorqueExample:
    def __init__(self, router, router_real_time):
        ...
        self.model_pin = pin.buildModelFromUrdf("/home/xht/catkin_ws/src/project/gen3_lite.urdf")
        ...
        self.torque_current_constant = self.torque_constant*self.gear_ratio*self.gear_efficiency
        ...
        for handle in device_handles.device_handle:
            print(handle.device_type)
            if handle.device_type == Common_pb2.BIG_ACTUATOR or handle.device_type == Common_pb2.SMALL_ACTUATOR or handle.device_type == Common_pb2.MEDIUM_ACTUATOR:
                print(handle.device_type)
    ...
    def InitCyclic(self, sampling_time_cyclic, t_end, print_stats):
        ...
        if base_feedback:
            ...
            device_id = 2  # first actuator as id = 1
            
            self.SendCallWithRetry(self.actuator_config.SetControlMode, 3, control_mode_message, 1)
            self.SendCallWithRetry(self.actuator_config.SetControlMode, 3, control_mode_message, 2)
            self.SendCallWithRetry(self.actuator_config.SetControlMode, 3, control_mode_message, 3)
            self.SendCallWithRetry(self.actuator_config.SetControlMode, 3, control_mode_message, 4)
            self.SendCallWithRetry(self.actuator_config.SetControlMode, 3, control_mode_message, 5)
            self.SendCallWithRetry(self.actuator_config.SetControlMode, 3, control_mode_message, 6)
    ...
    def RunCyclic(self, t_sample, print_stats):
        ...
        init_joint_position_degree = np.array([q.position for q in self.base_feedback.actuators])

        joint_position = np.radians(init_joint_position_degree)
        for i in range(6):
            if joint_position[i]>np.pi:
                joint_position[i]-=2*np.pi

        init_joint_position = joint_position
        init_joint_velocity = np.zeros_like(init_joint_position)

        init_torque = np.array([q.torque for q in self.base_feedback.actuators])
        init_current = np.array([q.current_motor for q in self.base_feedback.actuators])
        ...
        error_integral = np.zeros_like(joint_position)
        ...
        while not self.kill_the_thread:
            ...
            if (t_now - t_cyclic) >= t_sample:
                ...
                # torque current_motor

                joint_position = np.array([q.position for q in self.base_feedback.actuators])
                joint_position = np.radians(joint_position)
                for i in range(6):
                    if joint_position[i]>np.pi:
                        joint_position[i]-=2*np.pi


                joint_velocity =  np.array([q.velocity for q in self.base_feedback.actuators])
                joint_velocity = np.radians(joint_velocity)
                joint_torque = np.array([q.torque for q in self.base_feedback.actuators])
                joint_current = np.array([q.current_motor for q in self.base_feedback.actuators])
                ...
                error = init_joint_position - joint_position
                error_integral += error*0.001
                g = pin.rnea(self.model_pin,self.data_pin,joint_position,np.zeros_like(joint_position),np.zeros_like(joint_position))

                tau =  g + Kp*error + Kd*(init_joint_velocity-joint_velocity) + Ki * error_integral


                print("--------")
                print(f"position  :{joint_position}")
                print(f"velocity  : {joint_velocity}")
                print(f"torque    : {joint_torque}")
                # print(f"torque_sim: {joint_current*self.torque_current_constant}")

                tau_limit = np.array([10,14,10,7,7,7])
                for i in range(6):
                    if tau[i]<-tau_limit[i]:
                        tau[i] = -tau_limit[i]
                    elif tau[i]>tau_limit[i]:
                        tau[i] = tau_limit[i]

                print(f"current   : {joint_current}")
                print(f"error     : {error}")
                print(f"gravity   : {g}")

                print(f"cmd  tau  : {tau}")

                current_command = tau/self.torque_current_constant
                print(f"cmd cur   : {current_command}")
...
def main():
    ...
    parser.add_argument("--duration", type=int, help="example duration, in seconds (0 means infinite)", default=100)
